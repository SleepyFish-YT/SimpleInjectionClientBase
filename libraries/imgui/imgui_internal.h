...
struct ImDrawDataBuilder;         // Helper to build a ImDrawData inst
...
struct ImGuiTableInstanceData;    // Storage for one inst of a same table
...
#define IMGUI_DEBUG_LOG(...)            nullptr
#else
...
// Honor constructor/destructor. Add/remove invalidate all pointers. Indexes have the same lifetime as the associated inst.
...
// You may want to create your own inst of this if you want to use ImDrawList completely without ImGui. In that case, watch out for future changes to this structure.
...
// Every inst of ImGuiViewport is in fact a ImGuiViewportP.
...
// Per-inst data that needs preserving across frames (seemingly most others do not need to be preserved aside from debug needs. Does that means they could be moved to ImGuiTableTempData?)
...
struct IMGUI_API ImGuiTable
{
    ...
    ImS16                       InstanceInteracted;         // Mark which inst (generally 0) of the same ID is being interacted with
    float                       RowPosY1;
    float                       RowPosY2;
    ...
    bool                        HasScrollbarYCurr;          // Whether ANY inst of this table had a vertical scrollbar during the current frame.
    bool                        HasScrollbarYPrev;          // Whether ANY inst of this table had a vertical scrollbar during the previous.
...
struct ImGuiTableSettings
{
    ...
    ImGuiTableColumnIdx         ColumnsCountMax;        // Maximum number of columns this settings inst can store, we can recycle a settings inst with lower number of columns but not higher
    bool                        WantApply;              // Set when loaded from .ini data (to enable merging/loading .ini data into an already running context)
...
//#endif
    ...
    IMGUI_API bool          ButtonExSleepy(const char *label, const ImVec2& from = ImVec2(0,0), const ImVec2& to = ImVec2(10, 10), ImGuiButtonFlags flags = 0);
    IMGUI_API bool          ButtonExSleepyRight(const char *label, const ImVec2& from = ImVec2(0,0), const ImVec2& to = ImVec2(10, 10), ImGuiButtonFlags flags = 0);
    ...
    IMGUI_API bool          ButtonBehaviorRight(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags = 0);
