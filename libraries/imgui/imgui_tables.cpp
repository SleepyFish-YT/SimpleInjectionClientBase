...
bool    ImGui::BeginTableEx(const char *name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width) {
    ...
    // Instance data (for inst 0, TableID == TableInstanceID)
    ImGuiID instance_id;
...
// + 0 (for ImGuiTable inst, we are pooling allocations in g.Tables)
...
// Clear and initialize empty settings inst
...
void ImGui::DebugNodeTable(ImGuiTable* table) {
    ...
    if (!open)
        return;
    if (table->InstanceCurrent > 0)
        ImGui::Text("** %d instances of same table! Some data below will refer to last inst.", table->InstanceCurrent + 1);
